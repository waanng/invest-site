...
└─────────────────┘     └──────────────┘     └──────────────┘
         ...
         ▼                       ▼                     ▼
┌─────────────────┐     ┌──────────────┐     ┌──────────────┐
│ Alpha Vantage   │     │  失败重试    │     │ GitHub Pages │
│ (备用数据源)    │     │  备用切换    │     │ 自动部署     │
└─────────────────┘     └──────────────┘     └──────────────┘
```

**特性**:
- **双数据源**: Yahoo Finance（主）+ Alpha Vantage（备）
- **智能重试**: 指数退避 + 错误检测
- **详细日志**: 每步执行都有详细输出
...
| 数据类型 | 来源 | 代码 | 更新频率 |
|---------|------|------|----------|
| 黄金期货 | Yahoo Finance | GC=F | 每日 21:30 CST |
| 黄金波动率 | Yahoo Finance | ^GVZ | 每日 21:30 CST |
| 黄金现货 | Alpha Vantage | XAUUSD | 备用数据源 |
| 宏观经济 | 国家统计局 | - | 月度手动录入 |
...
3. **常见问题**
   - Yahoo Finance 限流 → 自动切换到 Alpha Vantage
   - API Key 未设置 → 在 GitHub Secrets 中配置
